...
* Author nya Radya, Farid, M.hadi.firmansya,Nazwa, MrDevils, dan WEM
* Tambahin ya zhayank
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6285746897368:+62 857-4689-7368\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '#'
blocked = []
limitawal = '30'
cr = '*🤖☘️ᴊᴀᴄᴋ BoT Verified🤖☘️*'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6285746897368@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6285746897368@s.whatsapp.net'
                        const isEventon = isGroup ? event.includes(from) : false
                        const isRegister = checkRegisteredUser(sender)
                        pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefined

                        // Options
                        const botName = '🤖☘️ᴊᴀᴄᴋ BoT☘🤖'
                        const ownerName = 'WEM'
                        const BarBarKey = 'WEMPY240905'
                        const vhtear = 'WEMPYBOT'
                        const tobz = 'BotWeA'
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 BOT BC GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 BOT BC GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
